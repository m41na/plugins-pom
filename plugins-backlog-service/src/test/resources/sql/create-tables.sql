--recreate tables--
drop table if exists tbl_backlog_item;
drop table if exists tbl_backlog_list;
...
--create tbl_backlog_list table--
create table if not exists tbl_backlog_list(
...
--create tbl_backlog_item table--
create table if not exists tbl_backlog_item(
  item_id integer primary key,
  item_name integer not null,
  is_done boolean not null default false,
  fk_list_id integer not null,
  item_created_ts text not null DEFAULT (datetime('now') ),
  foreign key (fk_list_id) references tbl_backlog_list(list_id)
);
