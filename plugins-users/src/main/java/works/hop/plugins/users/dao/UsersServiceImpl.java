...
public class UsersServiceImpl implements UsersService {
	...
	public PlugResult<User> create(User user) {
		...
		return (res > 0) ? new PlugResult<>(user, "ok") : new PlugResult<>("failed to create new user");
	}
	...
	public PlugResult<User> find(Long id) {
		...
		try {
			...
			return user != null ? new PlugResult<>(user, "ok") : new PlugResult<>("could not find user by id");
	...
	public PlugResult<List<User>> findUsers(int start, int size) {
		...
		try {
			...
			return users != null ? new PlugResult<>(users, "ok") : new PlugResult<>("could not find users");
		} catch (DataAccessException e) {
			return new PlugResult<>(e.getMessage());
		}
	}
