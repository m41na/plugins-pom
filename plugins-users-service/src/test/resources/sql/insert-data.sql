...
--create todo_list--
insert into tbl_todo_list (list_id, list_title, list_owner, list_created_ts)
select 1, "outdoors", 1, datetime('now')
where not exists (select * from tbl_todo_list where list_id = 1);

insert into tbl_todo_list (list_id, list_title, list_owner, list_created_ts)
select 2, "grocery", 2, datetime('now')
where not exists (select * from tbl_todo_list where list_id = 2);

insert into tbl_todo_list (list_id, list_title, list_owner, list_created_ts)
select 3, "workout", 1, datetime('now')
where not exists (select * from tbl_todo_list where list_id = 3);

insert into tbl_todo_list (list_id, list_title, list_owner, list_created_ts)
select 4, "school", 1, datetime('now')
where not exists (select * from tbl_todo_list where list_id = 4);

--create todo_items--
insert into tbl_todo_item (item_id, item_name, is_done, fk_list_id, item_created_ts)
select 1, 'grill', 'false', 1, datetime('now')
where not exists (select * from tbl_todo_item where item_id = 1);

insert into tbl_todo_item (item_id, item_name, is_done, fk_list_id, item_created_ts)
select 2, 'charcoal', 'true', 1, datetime('now')
where not exists (select * from tbl_todo_item where item_id = 2);

insert into tbl_todo_item (item_id, item_name, is_done, fk_list_id, item_created_ts)
select 3, 'milk', 'false', 2, datetime('now')
where not exists (select * from tbl_todo_item where item_id = 3);

insert into tbl_todo_item (item_id, item_name, is_done, fk_list_id, item_created_ts)
select 4, 'bread', 'false', 2, datetime('now')
where not exists (select * from tbl_todo_item where item_id = 4);

insert into tbl_todo_item (item_id, item_name, is_done, fk_list_id, item_created_ts)
select 5, 'bike', 'false', 3, datetime('now')
where not exists (select * from tbl_todo_item where item_id = 5);

insert into tbl_todo_item (item_id, item_name, is_done, fk_list_id, item_created_ts)
select 6, 'sneakers', 'true', 3, datetime('now')
where not exists (select * from tbl_todo_item where item_id = 6);
